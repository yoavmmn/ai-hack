  ...
  getInvestorsBySectors(sectors) {
    ...
    return Investors.find({}).then((docs) => {
      ...
      this.state.investors = _.filter(this.state.investors, (investor) => {
        if (investor.sectors.length < sectors.length) {
          for (var i=0; i<investor.sectors.length; i++) {
            if (sectors.indexOf(investor.sectors[i]) != -1) {
              return true;
            }
          }
          return false;
          ...
          for (var i=0; i<sectors.length; i++) {
            if (investor.sectors.indexOf(sectors[i]) != -1) {
              return true;
            }
          }
          return false;
